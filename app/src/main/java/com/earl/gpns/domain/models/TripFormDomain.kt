    ...
    class Base (
        ...
        override fun <T> map(mapper: TripFormDomainToUiMapper<T>) : T {
            return if (companionRole == COMPANION_ROLE) {
                mapper.mapCompanionDetails(username, userImage, companionRole, from, to, schedule, details as CompanionFormDetailsDomain)
            } else {
                mapper.mapDriversDetails(username, userImage, companionRole, from, to, schedule, details as DriverFormDetailsDomain)
            }
