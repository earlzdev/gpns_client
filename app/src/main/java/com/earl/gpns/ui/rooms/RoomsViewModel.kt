...
import com.earl.gpns.core.DeleteRoomCallback
import com.earl.gpns.core.LastMessageReadStateCallback
        ...
        viewModelScope.launch(Dispatchers.IO) {
            when(interactor.initChatSocketSession(token)) {
                is SocketOperationResultListener.Success -> {
                    interactor.observeNewRooms(callback, authoredMessagesReadCallback, removeRoomCallback).onEach { room ->
